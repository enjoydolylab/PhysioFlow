...
export default function GraphRuntimeRunnerPage({ data, onDone }) {
  ...
  useEffect(() => {
    ...
    saveQueueRef.current.catch(() => {}).then(() => persistFinishedSession(finished, { saveSession, clearCurrentRun: () => clearCurrentRun({ strict: true }) })).then(result => { setSaved(true); setCleanupError(result.cleanupError); }).catch(error => setSaved(error.message || 'Save failed'));
  }, [data.session, data.hosted, events, protocol, responses, runtime, saveAttempt, legacyStimulusOrder]);
