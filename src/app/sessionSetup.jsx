...
export function GraphSessionSetup({ protocol: p, onBack, onStart, storageInfo, onChooseDataDirectory, onGuide, guideOpen, guideTab, onCloseGuide }) {
  ...
  const requiredDevices = [...new Set(deviceNodes.filter(node => node.config.deviceRequired !== false).map(node => node.config.deviceConnectorId))];
  ...
  return <main><Header onGuide={onGuide} /><div className="narrow">
    ...
    <nav aria-label="Collection workflow" className="setup-note">Design → <strong aria-current="step">Prepare</strong> → Run → Review data</nav>
    <span className="eyebrow">PREPARE COLLECTION</span>
    <h1>{protocolNameOf(p)}</h1>
    <p>Check the participant, protocol, media and storage before opening the experiment.</p>
    <section className="setup-note" aria-label="Run configuration"><b>{isFormal ? 'Formal collection · frozen version' : 'Test run · draft version'} {p.version.number}</b><p>{isFormal ? 'This session uses the frozen configuration below.' : 'This session will be recorded as a preview. Return to the protocol and freeze a version for formal collection.'}</p><code style={{ overflowWrap: 'anywhere' }}>{p.freeze?.configHash || 'No frozen configuration hash'}</code></section>
    ...
    <section className="setup-note" aria-label="Device requirements"><b>Devices · {deviceNodes.length ? 'connection pending' : 'none configured'}</b><p>{deviceNodes.length ? `Required: ${requiredDevices.join(', ') || 'none'}. Device connection is checked when you begin the experiment; configuration does not confirm connectivity or timing accuracy.` : 'This protocol has no configured device connection.'}</p>{deviceNodes.length > 0 && <p>Configured: {[...new Set(deviceNodes.map(node => node.config.deviceConnectorId))].join(', ')}</p>}</section>
    <div className={`setup-note storage-gate ${storageBlocked ? 'blocked' : storageInfo?.selected ? 'ready' : 'preview'}`}>
      ...
      <p>{storageBlocked ? 'Select a local data folder before formal collection.' : storageInfo?.selected ? 'Checkpoints and final session data are written to this folder. Unsubmitted questionnaire answers and task progress may not be recoverable.' : 'This test run uses browser-managed storage. Export a data package after the run to keep a separate copy.'}</p>
      {storageBlocked && onChooseDataDirectory && <button onClick={onChooseDataDirectory}>Select local data folder</button>}
    </div>
    <div role="status" className="setup-note"><b>{blockers.length ? 'Before you continue' : 'Ready to open the experiment'}</b>{blockers.map(message => <p key={message}>{message}</p>)}</div>
    <button className="primary wide" disabled={blockers.length > 0} onClick={() => onStart({
